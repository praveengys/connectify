  ...
  const handleAction = (booking: DemoBooking, action: 'approve' | 'deny') => {
    ...
    switch (action) {
      case 'approve':
        ...
        description = `This will schedule the demo for ${booking.name} on ${format(new Date(booking.date), 'PPP')} at ${booking.startTime}.`;
    ...
    <>
      <Card>
        <CardHeader>
          ...
          <CardDescription>Manage all demo requests from users.</CardDescription>
        </CardHeader>
        <CardContent>
          <Table>
            <TableHeader>
            ...
            <TableBody>
              ...
              {bookings.length === 0 && (
                <TableRow><TableCell colSpan={5} className="h-24 text-center">No booking requests found.</TableCell></TableRow>
              )}
            </TableBody>
          </Table>
        </CardContent>
